...
def read_data(split, data_dir='data'):
    # 读取数据
    assert split in ['relations_data_all', 'relations_data_dev', 'neo_face_data_one', 'neo_face_data', 'relation_all_trp']
    step_index = 1
...
def exe_fun(ep, rt):
    ...
    model_textgcn = TextGCN(len(word_2_id), embedding_num, hidden_num, len(tag_2_id), n_layers=4)
    opt_textgcn = torch.optim.Adam(model_textgcn.parameters(), lr=rt)
    model_textgcn = model_textgcn.to(device)

    text_file = open('logs/relation_param_rt_2_' + data_file + '.txt', mode='a', encoding='utf-8')  # 打开文件，文件存在则打开，不存在则创建后再打开
    for e in range(ep):
        model_textgcn.train()
        ...
        print('------------------------------------------------------------------------------')
    dp_json = {'epoch': ep, 'rt': rt, 'precision_text_gcn': precision_text_gcn, 'recall_text_gcn': recall_text_gcn, 'f1_text_gcn': f1_text_gcn}
...
if __name__ == '__main__':
    ...
    for rt in [0.000000005, 0.00000001, 0.00000005, 0.0000001, 0.0000005, 0.000001, 0.000005, 0.00001, 0.00005, 0.0001, 0.0005, 0.001, 0.005, 0.01, 0.05]:  #
