...
from entity_rel_ptm_mfgcn.predicate_entity_textgcn_cut import Bert_Model
...
if __name__ == '__main__':
    ...
    opt_bert = torch.optim.Adam(model_bert.parameters(), lr=0.00001)
    model_bert = model_bert.to(device)

    text_file = open('logs/cut_' + data_file + '.txt', mode='a', encoding='utf-8')  # 打开文件，文件存在则打开，不存在则创建后再打开
